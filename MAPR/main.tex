...
To have a comparison with the related works, we have calculated the same quality metrics for regression problem, \textit{such as,} $r^2$, RMSE, explained variance (EV), and Pearson correlation using \textit{scikit-learn} \cite{pedregosa2011scikit}. The accuracy of the last model has been shown in Table.\ref{tab3}. Comparing with the result of Cintas \cite{cintas2016automatic}, most of our metrics are better than their result (except RMSE). 

\begin{table}[htbp]
\caption{The performance of model on regression's quality metrics}
\begin{center}
\begin{tabular}{|c|p{1cm}|p{1cm}|p{1cm}|p{1cm}|}
\hline
Metric & \textbf{$r^2$} & \textbf{RMSE} & \textbf{EV} & \textbf{Distance} \\ \hline
Value & $0.9952$ & $3.8512$ & $0.9952$ & $0.9974$ \\ \hline
\end{tabular}
\label{tab3}
\end{center}
\end{table}
