...
For pre-train step, the architecture with elementary blocks was continued to be selected. Basically, the architecture parameters (number filters, size of filter,padding, stride) are used like working on beetle's parts. We have just modified on size of the input of network model to $96 \times 96$ and the output of the last FC layers to correspond to the number of landmarks. Besides, the hyper-parameters (learning rate, momentum) are also kept the same as when training from scratch on each part of beelte. However, we have increased the number of epochs to $10,000$ instead of $5,000$ to achieve better learning on the parameters.

\subsection{Fine-tuning on each beetle's part}

Our model architecture has been pre-trained on facial keypoints dataset with $30$ outputs ($15$ landmarks). Then, the parameters have been transfered to fine-tune on the images of each beetle's part. The fine-tuning stage has been done in two scenarios: with and without freeze process. In the first scenario, the parameters values of layers were continued to update during fine-tuning; while in the second scenario, the parameters of $3$ convolutional layers in the elementary blocks have been frozen.
...
During fine-tuning, cross-validation has been applied to select data for training and valiation. At the end of process, the landmarks are predicted on the test images. Then, the average distances have been calculated based on the distance (in pixels) between predicted and corresponding manual landmarks. Tables. \ref{} show the average distance by landmarks on pronotum, elytra, head, left and right mandible images, respectively. \textbf{From scratch} columns remind the previously average distances. \textbf{Fine-tune freeze} and \textbf{Fine-tune} columns present the new average distances after applying fine-tuning on each part without and with freeze process, sequentially. The average distance at each landmark has decreased and clearly that the result of predicted landmarks with the help of fine-tuning is more precise than training from scratch.
