...
Training a network from scratch is not the only way to work in Deep learning. It is possible to initialize parameter values with values extract from another experiment with another dataset. It is called transfer learning \cite{torrey2009transfer}. Transfer learning aims
to address the problem when the distribution of the training
data from the source domain is different from that of the future data from the target domain \cite{lin2016homemade}. In order to improve our results, we have broadened model with this technique. The obtained parameter values have been used in another context to classify other datasets \cite{margeta_mri}. The name of this procedure to re-use parameters to train a model is currently called \textbf{fine-tuning}.

Fine-tuning does not only replace and retrain the top of the model on
the new dataset but also fine-tunes the weights of a trained
model by continuing the backpropagation. Unfortunately,
some preliminary tests have shown that re-using ImageNet features
...
\begin{figure}[htbp]
	\centerline{\includegraphics[scale=0.2]{images/face_dataset}}
	\caption{Some face images in the dataset and ground truth position of the landmarks.}
	\label{figaface}
\end{figure}

For pre-training step, the architecture with elementary blocks is continued to be selected. Basically, the architecture parameters (number filters, size of filter,padding, stride) are used like working on beetle's parts. We have just modified on size of the input of network model to $96 \times 96$ and the output of the last FC layers to correspond to the number of landmarks. Besides, the hyper-parameters (learning rate, momentum) are also kept the same as when training from scratch on each part of beelte. However, we have increased the number of epochs to $10,000$ instead of $5,000$ to achieve better learning on the parameters.

\subsection{Fine-tuning on each beetle's part}

Our model architecture has been pre-trained on facial keypoints dataset with $30$ outputs ($15$ landmarks). Then, the parameters have been transfered to fine-tune on the images of each beetle's part. The fine-tuning stage has been done with continuing the backpropagation to update the layers parameters.

In another scene, to be able to use the same parameters for fine-tuning process, the fine-tuning images have been cropped to suppress the part which are empty, to obtain a new image size of $192 \times 192$; and a modification was made on stride properties of the first convolutional layer (changing from $1$ to $2$).

During fine-tuning, cross-validation has been applied to select data for training and valiation. At the end of process, the landmarks are predicted on the test images. Then, the average distances have been calculated based on the distance (in pixels) between predicted and corresponding manual landmarks. Tables. \ref{cmppronotum}, \ref{cmptete}, \ref{cmpelytre}, \ref{cmpmg}, and \ref{cmpmd} show the comparison of the average distances by landmarks between training from scratch and fine-tuning process on pronotum, elytra, head, left and right mandible images, respectively. \textbf{From scratch} columns remind the previously average distances. \textbf{Fine-tune} columns present the new average distances after applying fine-tuning on each part. The average distance at each landmark has decreased and clearly that the result of predicted landmarks with the help of fine-tuning is more precise than training from scratch. Fig. \ref{figpdl} shows the predicted landmarks in one case of each part.
...
\begin{table}[htbp]
	...
	\begin{tabular}{|c|c|c|}
		...
		1 & \textcolor{green}{\textbf{4.00 }}& \textcolor{green}{\textbf{2.99}}\\ \hline
		2 & 4.48 & 3.41  \\ \hline
		3 & 4.30  & 2.98 \\ \hline
		4 & 4.39  & 3.54\\ \hline
		5 & 4.29  & 3.37 \\ \hline
...
\begin{table}
	\begin{minipage}[t]{0.45\textwidth}
		...
		3 & 5.38  & 4.73 \\ \hline
		4 & 5.03  & 4.11 \\ \hline
		\textbf{5} & 4.18  & 2.76\\ \hline
		\textbf{6} & \textcolor{green}{\textbf{4.45}}  & 3.50 \\ \hline
		7 & 4.79  & 3.92 \\ \hline
		8 & 4.53  & \textcolor{green}{\textbf{3.40}}\\ \hline
		9 & 5.14  & 4.17 \\ \hline
		10 & 5.06  & 3.94\\ \hline
	\end{tabular}
	...
	\begin{minipage}[t]{0.45\textwidth}
		...
		\begin{tabular}{|c|c|c|}
			\hline
			\textbf{$\#$LM} & \textbf{From scratch} & \textbf{Fine-tune} \\ \hline
			\textbf{1} & \textcolor{green}{\textbf{3.87}} & 3.21  \\ \hline
			2 & 3.97 & 3.28 \\ \hline
			3 & 3.92  & 3.20\\ \hline
			4 & \textcolor{green}{\textbf{3.87}}  & \textcolor{green}{\textbf{3.22}} \\ \hline
			5 & 4.02  & 3.31 \\ \hline
			\textbf{6} & 4.84  & 4.21\\ \hline
			7 & 5.21  & 4.54 \\ \hline
			8 & \textcolor{red}{\textbf{5.47}}  & 4.76\\ \hline
			\textbf{9} & 5.27  & \textcolor{red}{\textbf{4.55}} \\ \hline
			10 & 4.07  & 3.39 \\ \hline
			11 & 3.99  & 3.29 \\ \hline
		\end{tabular}
...
\begin{table}
	\begin{minipage}[t]{0.45\textwidth}
		...
		\begin{tabular}{|c|c|c|}
			\hline
			\textbf{$\#$LM} & \textbf{From scratch} & \textbf{Fine-tune} \\ \hline
			1 & \textcolor{red}{\textbf{9.13}} & \textcolor{red}{\textbf{5.28}} \\ \hline
			2 & \textcolor{green}{\textbf{6.72}} & \textcolor{green}{\textbf{4.05}} \\ \hline
			3 & 6.87 & 4.01 \\ \hline
			4 & 6.77 & 4.02 \\ \hline
			5 & 7.13 & 3.92 \\ \hline
			6 & 6.94 & 3.88 \\ \hline
...
\textbf{$\#$LM} & \textbf{From scratch} & \textbf{Fine-tune} \\ \hline
	...
	2 & 7.17 & 4.06\\ \hline
	3 & 7.24 & 3.97\\ \hline
	4 & \textcolor{green}{\textbf{7.04}} & \textcolor{green}{\textbf{3.87}} \\ \hline
	5 & 7.16 & 4.05  \\ \hline
	6 & 7.57 & 3.82  \\ \hline
	7 & 7.43 & 3.77  \\ \hline
	8 & 7.66 & 3.87  \\ \hline
	...
	12 & 8.16 & 4.42 \\ \hline
	13 & 8.89 & 4.87 \\ \hline
	14 & 9.18 & \textcolor{red}{\textbf{4.93}} \\ \hline
	15 & 8.79 & 4.46 \\ \hline
	16 & 8.31 & 4.17 \\ \hline
	17 & 8.29 & 4.57 \\ \hline
	18 & 8.89 & 5.89 \\ \hline
	...
	\end{minipage}
\end{table}
In order to get a better view of predicting results, we have calculated other statistical indicators such as median, standard error, minimum value and maximum value on each landmark based on the distances between predicted and corresponding manual landmarks. Besides, the distribution of the distances on each landmark of each part have been taked into account. All statistical values and the distribution graphs are presented in \ref{appdixA} and \ref{appdixB}.
...
\label{appdixA}
Table \ref{a1}, \ref{a2}, \ref{a3}, \ref{a4}, and \ref{a5} show the statistical values on each part. The green and red numbers present the best and the worse values on each statistical indicators, respectively.  
\begin{table}[htbp]
