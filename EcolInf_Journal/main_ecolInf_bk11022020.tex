...
EB-Net has been firstly trained and tested on the pronotum images before applying to other parts beetle (head and elytra). The first evaluation is typically considered the losses during the training process. Figure \ref{figdlosses} shows the losses of training and validation processes of one round on pronotum images. The blue curves are training losses, and the green curves are validation losses. Clearly, the losses are far from the beginning but become more close at the end of the process. The over-fitting has not been appeared any more. Therefore, we can assume that elementary blocks have properly worked to prevent over-fitting in our case. For these reasons, we have decided to use EB-Net for producing the landmarks on beelte images.
