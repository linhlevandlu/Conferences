...
\begin{figure}[h!]
    \centering
    \subcaptionbox{. The $1^{st}$ landmark \label{figc4lm1dist}}{\includegraphics[width=0.48\textwidth]{./images/fs_lm1}}\hspace{0.5cm}
\subcaptionbox{. The $6^{th}$ landmark \label{figc4lm6dist}}{\includegraphics[width=0.48\textwidth]{./images/fs_lm6}}
...
region instead of down-sampling our pictures. Moreover, removing the background pixels can limit the effect of un-useful image areas. So, the new beetle images are finally set to $192 \times 192$ pixels. The EB-Net parameters will be settled to take into account these values between the pre-training and fine-tuning steps. To declare the modification, we mention in a stride value of the first convolutional layer equals to $2$ (as the usual way to do \cite{}). 
