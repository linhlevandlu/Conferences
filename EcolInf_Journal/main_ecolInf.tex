...
\label{subsec22}
Our initial trials were inspired by AlexNet architecture \cite{krizhevsky2012imagenet}. These models have been designed by combining in sequence the classical layers, e.g., convolutional (CONV), max pooling (POOL) and fully-connected (FC) layers. However, the results were unconsidered good enough to utilize them. The particular problem came from the appearance of over-fitting during the training process. After experiments, we have defined a concept of Elementary Block (EB) and employed it to build the network model. Figure \ref{figeblock} illustrates the order of the classes in an EB. It is defined as a sequence of a CONV layer, a maximum POOL layer, and a Dropout layer. 

\begin{figure}[h!]
	\centering
	\includegraphics[width=0.46\textwidth]{images/eblock}
	\caption{The components of an elementary block}
	\label{figeblock}
\end{figure}

In an Elementary Block, the convolution layer is used to extract the high-level features by applying different filters on the input. The appearance of POOL layer after the CONV layer is a frequent occurrence in CNN \cite{krizhevsky2012imagenet, ciregan2012multi, cintas2016automatic}. This work towards to two advantages: reducing the spatial size of the representation to decrease the number of parameters as well as saving the computing time, and to prevent the over-fitting during the training process. The dropout layer is usually inserted between the FC layers to prevent over-fitting as we have seen in the success stories \cite{lecun2015deep, krizhevsky2012imagenet}. But in our case, we have included them in the extracting features blocks (after pooling layers) to create some kinds of image noise augmentation. The objective of this work is to provide more studied cased in each training iteration. Furthermore, the dropout layer will randomly drop some connections during the training process. It makes the network thinner than the original one (fewer parameters), and training a network with dropout layer is equivalent to train a set of thinner networks. These works have significantly reduced over-fitting and given more considerable improvements than other regularization methods \cite{srivastava2014dropout}.
...
For our critical objective, we have initially assembled three Elementary Blocks to create the model architecture, so-called Elementary Blocks Network (EB-Net). The used parameters of layers in elementary blocks have been set as follows: the depths of CONV layers are set to increase with a small kernel from the first to the third block. We have chosen $32, 64, 128$ for the depths and $3 \times 3$, $2 \times 2$, $2 \times 2$ for kernel's sizes, respectively.  The POOL layers have been designed with a frequent form for every block: a filter of $2 \times 2$ with a stride of $2$ pixels. With these parameter values, the spatial size of the image will be halved after every block. The probabilities of the dropout layers, which were added after the pooling layers, are set increasing: $0.1, 0.2$, and $0.3$, respectively. In order to extract the global relationship between the features and to provide the prediction, three fully-connected layers have been added after the combination of elementary blocks. The first FC layer takes all features from the last block as the input for computing. Then, the computed results will be passed through the activation functions to provide the outputs. The second FC layer receives the outputs of the first FC layer as the input. The operations at this layer are the same than the first one. The third FC layer accepts the outputs of the second FC layer, then they will be used to compute the input. The number of outputs at each FC layers is set to $1000, 1000$, and $X$, respectively. The number $X$ equals to two times the number of landmarks that we want to predict. For example, to predict $8$ landmarks on pronotum, we have set $16$ as the outputs of the last FC layer. Additional, a dropout layer with the probability equals to $0.5$ has been inserted between the first and the second fully-connected layers. Figure \ref{figebnet} shows the structure of the EB-Net.
...
EB-Net has been trained and tested on the pronotum images before applying to other parts beetle (head and elytra). The first evaluation is typically considered the losses during the training process. Figure \ref{figdlosses} shows the losses of training and validation processes of one round on pronotum images. The blue curves are training losses, and the green curves are validation losses. Clearly, the losses are far from the beginning but become more close at the end of the process. The over-fitting has not been appeared any more. Therefore, we can assume that elementary blocks have properly worked to prevent over-fitting in our case. For these reasons, we have decided to use EB-Net for producing the landmarks on beelte images.

\begin{figure}[h!]
    \centering
	\includegraphics[width=0.46\textwidth]{./images/loss_v16}
...
\begin{figure}[h!]
    \centering
    \subcaptionbox{. The $1^{st}$ landmark (pronotum)}{\includegraphics[width=0.45\textwidth]{./images/fs_lm1}}\hspace{0.5cm}
\subcaptionbox{. The $6^{th}$ landmark (pronotum)}{\includegraphics[width=0.45\textwidth]{./images/fs_lm6}}~\\
\subcaptionbox{. The $6^{th}$ landmark (head)}{\includegraphics[width=0.45\textwidth]{./images/head6_fs}}\hspace{0.5cm}
\subcaptionbox{. The $1^{st}$ landmark (head)}{\includegraphics[width=0.45\textwidth]{./images/head1_fs}}~\\
\subcaptionbox{. The $1^{st}$ landmark (elytra)}{\includegraphics[width=0.45\textwidth]{./images/elytre1_fs}}\hspace{0.5cm}
\subcaptionbox{. The $8^{th}$ landmark (elytra)}{\includegraphics[width=0.45\textwidth]{./images/elytre8_fs}}
...
\begin{figure}[h!]
	\centering
    \subcaptionbox{. Pronotum}{\includegraphics[width=0.31\textwidth]{images/Prono_001_2}}~~
	\subcaptionbox{. Head}{\includegraphics[width=0.31\textwidth]{images/Tete_006_2}}~~
	\subcaptionbox{. Elytra}{\includegraphics[width=0.31\textwidth]{images/Elytre007_2}}
